...
<h2><a href="https://leetcode.com/problems/dungeon-game" target = "_blank">174. Dungeon Game</a></h2><h3>Hard</h3><hr><p>The demons had captured the princess and imprisoned her in <strong>the bottom-right corner</strong> of a <code>dungeon</code>. The <code>dungeon</code> consists of <code>m x n</code> rooms laid out in a 2D grid. Our valiant knight was initially positioned in <strong>the top-left room</strong> and must fight his way through <code>dungeon</code> to rescue the princess.</p>
...
public:
    int calculateMinimumHP(vector<vector<int>>& arr) {
        ...
        for (int i = n - 1; i >= 0; i--) {
            for (int j = m - 1; j >= 0; j--) {
                int current = min(dp[i + 1][j], dp[i][j + 1]) - arr[i][j];
                dp[i][j] = max(1, current);
            }
        }
